...
class EmbeddingManager(nn.Module):
    def __init__(
        ...
        self.gamma = gamma
        print(f"embedding manager gamma: {gamma}")
        print(f"initializing word: {initializer_words}")
    ...
    def forward(
        ...
        for placeholder_string, placeholder_token in self.string_to_token_dict.items():
            ...
            # trainable momentum embedding term
            ...
            if self.max_vectors_per_token == 1: # If there's only one vector per token, we can do a simple replacement
                ...
                print(placeholder_embedding - embedded_text[placeholder_idx])
                print(torch.norm(placeholder_embedding - embedded_text[placeholder_idx]))
                assert torch.norm(placeholder_embedding - embedded_text[placeholder_idx]) > 0
                embedded_text[placeholder_idx] = self.gamma * (placeholder_embedding - embedded_text[placeholder_idx]) + embedded_text[placeholder_idx]
